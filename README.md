...
用斜杠命令触发：`/groundwork`、`/review-worker` 等，或直接描述任务让 Codex 自行选择。（注意 `@skill-name` 只对直接放进 `~/.codex/skills/` 顶层的独立 skill 有效；本插件的 skill 装在 plugin 缓存里，不在那个搜索路径，须用 `/` 斜杠命令。）审查交给 `review-worker`：Codex 的独立审查靠只读审查 skill 实现（其内置的 `review-agent` 即是此形态），review-worker 正是这个角色。仓库根的 `agents/reviewer.md` 是 Claude Code 专用 persona——用工具白名单从权限层锁死只读，这是 Claude Code 独有的加强；Codex 不加载它，也不需要，review-worker 已覆盖审查职责。
