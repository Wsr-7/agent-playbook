...
- `templates/agents/reviewer.md` → `~/.claude/agents/`：只读验收 reviewer agent——工具白名单不含编辑能力，maker/checker 分离由权限而非嘱咐保证（sync.ps1 自动安装到 Claude Code 侧）

Hook 是可选增强，不是依赖：平台不支持 hooks 时，spine 的开工规则会以 prose 方式兜底（开工先读 STATE.md 和 lessons）——概率性但通常有效；支持 hooks 的平台装上后升级为确定性注入。所有 skill 在无 hook 环境下功能完整。reviewer agent 同理：无自定义 agent 能力的平台退回 ship §7 的 prose 版 reviewer brief。
