...
The user has passed (or will pass) a markdown file of raw material. Read it end-to-end before proposing any beats. Do not edit the raw material file: it is read-only to this skill.

This is **exploit**: the exploring is done, the pile is fixed. Commit to a path through it and mine the pile to fill each beat.

If the user did not say where to save the article, ask once and remember the path. If the `writing` router already supplied the source or article path, use it and get on with the work.
...
If a candidate beat needs supporting material the pile does not contain, name the gap before writing the beat: ask the user for the missing material, or drop that candidate. Every event, quote, and example in a beat comes from the pile or from the user.
...
When the journey is complete, stop and report the article path.
